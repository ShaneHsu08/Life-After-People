...
export const generateVideoScript = async (language: string, imageDescription: string | null): Promise<string> => {
    ...
    try {
        const prompt = `You are a documentary scriptwriter for a short social media video. Write a compelling, dramatic voice-over script showing a single location as it decays over a million years after humans vanish. ${descriptionContext} The video will use six keyframes: 1 year, 100 years, 1,000 years, 10,000 years, 100,000 years, and 1 million years. Structure the script with scene descriptions for each keyframe (e.g., "[SCENE 1: 1 YEAR LATER]") and corresponding narration. Crucially, the narration for each scene must be very brief, designed to be read aloud in approximately 8 seconds (around 20-25 words). The tone should be awe-inspiring, slightly melancholic, and educational, in the style of the 'Life After People' documentary series. The script must be written in ${targetLanguage}.`;
